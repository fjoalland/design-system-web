 ...
 * License  : https://github.com/nico3333fr/van11y-accessible-hide-show-aria/blob/master/LICENSE
 */
(doc => {
    ...
    /* LChoquet - ajout d'une variable déterminant si le curseur est passé sur un tooltip depuis son déclencheur ou réciproquement */
    var isTooltipOrTriggerHovered = false;

    /** Effectue les actions pour cacher un tooltip sauf conditions
     * @param {tooltipLauncher} l'élément déclencheur du tooltip. Peut être le tooltip lui-même
     * @param {isTooltip} si le tooltipLauncher est le tooltip
     * @author LChoquet
     */
    function performHide(tooltipLauncher, isTooltip) {
        if (!isTooltipOrTriggerHovered) {
            let item = tooltipLauncher;
            if (!isTooltip) {
                item = findById(tooltipLauncher.getAttribute(ATTR_DESCRIBEDBY));
            }
            if (item) {
                        ...
                        if (hasClass(e.target, TOOLTIP_SIMPLE) === true) {
                            ...
                            if (eventName === 'mouseenter' || eventName === 'focus') {
                                ...
                                if (item) {
                                    ...
                                    isTooltipOrTriggerHovered = true; // LChoquet - indiquer qu'un déclencheur a été hovered
                                }
                                if ("tooltip" == tooltipLauncher.getAttribute('role')) {
                                    isTooltipOrTriggerHovered = true; // LChoquet - indiquer qu'un tooltip a été hovered
                                }
                            }
